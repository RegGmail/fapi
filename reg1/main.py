...
@app.get("/reg1/index")
def index():
    return casas_pendentes
    #return {"message": "Casa Automatizada"}

@app.post("/reg1/casa_conectar")
def signup(casa: str, cod: str):
    if ( casa == None and codigo == None):
        print ("---> erro")
        return {"message" : "casa invalida"}
    elif not casas_validas.get(casa) == None:
        print ("---> existe casa")
        return {"message": "casa conectada"}
    else:
        resid = Casa(casa_nome=casa, casa_codigo=cod)
        casas_pendentes[casa] = casa
        print ("---> OK", resid.casa_nome, resid.casa_codigo)
        return resid
